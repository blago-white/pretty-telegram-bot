...
class CallbackHandler:
    ...
    async def callback_handler(self, callback_query: aiogram.types.CallbackQuery):
        ...
        user_lang_code = self._database_operation_assistant.get_user_lang_code(user_id=user_id)

        type_logging, stage_logging = self._database_operation_assistant.get_logging_info(user_id=user_id).object[-2:]
...
class MessageHandler:
    ...
    async def start_handler(self, message: aiogram.types.Message, user_id: int):
        ...
        user_logging_condition = self._database_operation_assistant.get_logging_info(user_id=user_id)
        ...
        if not user_logging_condition.object:
            response = self._database_operation_assistant.add_user_entry(user_id=user_id,
            ...
            if logging:
                if logging_type not in MAIN_REGISTRATION_TYPE:
                    self._database_operation_assistant.change_state_logging(user_id=user_id, stop_logging=True)

                    for type_ in TYPES_MAIN_MESSAGES:
                        if type_ != PROFILE_MESSAGE_TYPE:
                            print(type_)
                            await self._bot_helper_scripts.delete_main_message(user_id=user_id, type_message=type_)
    ...
    async def handle_photo(self, message: aiogram.types.Message, user_id: int):
        user_loging_condition = self._database_operation_assistant.get_logging_info(user_id=user_id)
        ...
        if logging:
            ...
            if stage_logging == 5:
                ...
                if statement_response.status:
                    self._bot_helper_scripts.record_registration_info(user_id=user_id,
    ...
    async def text_handler(self, message: aiogram.types.Message, user_id: int):
        ...
        if self._database_operation_assistant.get_chatting_condition(user_id=user_id).object:
            await self._message_sender.send(user_id=message.from_user.id,
                                            description=message.text)

            return

        user_logging_data = self._database_operation_assistant.get_logging_info(user_id=user_id)
    ...
    async def restart_bot_handler(self, message: aiogram.types.Message, user_id: int):
        ...
        self._database_operation_assistant.del_user_annotations(user_id=user_id)
        await self._message_deleter.delete_message(user_id=user_id, idmes=message.message_id)
    ...
    async def change_language_handler(
        ...
        self._database_operation_assistant.change_user_lang(user_id=user_id, lang_code=message.text[1:3])
    ...
    async def end_chatting(self, message: aiogram.types.Message, user_id: int):

        current_chatting_condition = self._database_operation_assistant.get_chatting_condition(user_id=user_id)

        if current_chatting_condition.object:
            user_lang_code = self._database_operation_assistant.get_user_lang_code(user_id=user_id)

            self._database_operation_assistant.change_chatting_condition(user_id=user_id, new_condition=False)

            response_sending = await self._message_sender.send(user_id=user_id,
                                                               description=STATEMENTS_BY_LANG[
                                                                   user_lang_code].end_chatting)

            await self._bot_helper_scripts.render_profile(user_id=user_id, user_lang_code=user_lang_code)

            await self._bot_helper_scripts.start_delay(DEFAULT_DELAY)

            await self._message_deleter.delete_message(user_id=user_id, idmes=response_sending.object)
