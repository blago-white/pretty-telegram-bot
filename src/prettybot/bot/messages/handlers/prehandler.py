...
class MessagePreHandler:
    ...
    def prehandle(
        ...
        async def wrapper():
            if message.chat.type == 'private':
                ...
                await self.start_message_handler(handler_function=handler_func,
                                                 message=message,
    ...
    async def start_message_handler(self, handler_function, message: aiogram.types.Message, user_lang_code: str):
        try:
            await handler_function(message, message.from_user.id, user_lang_code)
